...
// Path ngựa (game-icons:horse-head, viewBox 512) — nhúng offline, không phụ thuộc mạng.
const HORSE_PATH =
...
interface LudoHorseProps {
  color: string;
  dark: string;
  light: string;
}

const EXTRUDE = 9; // số lớp đùn tạo chiều cao khối

/** Quân ngựa "tượng" 3D: nhiều lớp đùn xuống dưới tạo độ dày + mặt trên gradient + highlight. */
export function LudoHorse({ color, dark, light }: LudoHorseProps) {
  const uid = useId().replace(/:/g, '');
  const gBody = `hb${uid}`;
  const gHi = `hh${uid}`;
  const gSide = `hs${uid}`;
  return (
    <svg viewBox="0 0 512 512" className="ludo-horse" aria-hidden>
      <defs>
        ...
        <linearGradient id={gHi} x1="0" y1="0" x2="0" y2="1">
          ...
          <stop offset="36%" stopColor="#ffffff" stopOpacity="0.14" />
          <stop offset="58%" stopColor="#ffffff" stopOpacity="0" />
        </linearGradient>
      </defs>
      {/* các lớp đùn xuống dưới -> khối dày (vẽ từ đáy lên) */}
      {Array.from({ length: EXTRUDE }).map((_, i) => (
        <path
          key={i}
          d={HORSE_PATH}
          fill={`url(#${gSide})`}
          transform={`translate(0 ${(EXTRUDE - i) * 3.2})`}
        />
      ))}
      {/* mặt trên: thân ngựa gradient khối + viền tối */}
      <path d={HORSE_PATH} fill={`url(#${gBody})`} stroke={dark} strokeWidth="10" strokeLinejoin="round" />
      {/* highlight bóng phía trên */}
      <path d={HORSE_PATH} fill={`url(#${gHi})`} />
    </svg>
  );
}
